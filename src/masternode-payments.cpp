...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake, bool fZKSHStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    ...
    if (IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
        ...
        masternodePayments.FillBlockPayee(txNew, nFees, fProofOfStake, fZKSHStake);
    }
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake, bool fZKSHStake)
{
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (!pindexPrev) return;
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZKSHStake);

    CScript developmentPayee = GetScriptForDestination(CBitcoinAddress(Params().DevBlockRewardAddress()).Get());
    CAmount developmentPayment = GetDevelopmentPayment(pindexPrev->nHeight, blockValue, fZKSHStake);
    ...
    if (hasPayment) {
        if (fProofOfStake) {
            ...
            if (developmentPayment > CAmount(0)) {
        ...
        if (!fProofOfStake) {
            ...
            if (developmentPayment > CAmount(0)) {
                txNew.vout.resize(2);
                txNew.vout[1].scriptPubKey = developmentPayee;
                txNew.vout[1].nValue = developmentPayment;
            }
        }
    }
}
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    if (requiredDevelopmentPayment > CAmount(0)) {

...
#include "ui_zxdhcontroldialog.h"
...
std::set<std::string> ZXdhControlDialog::setSelectedMints;
std::set<CMintMeta> ZXdhControlDialog::setMints;

bool CZXdhControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZXdhControlDialog::COLUMN_DENOMINATION || column == ZXdhControlDialog::COLUMN_VERSION || column == ZXdhControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZXdhControlDialog::ZXdhControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZXdhControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZXdhControlDialog::~ZXdhControlDialog()
{
    delete ui;
}

void ZXdhControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXdhControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZXdhControlWidgetItem* itemDenom(new CZXdhControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZXdhControlWidgetItem *itemMint = new CZXdhControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZXdhControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXdhControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZXdh_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXdhControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZXdhControlDialog::GetSelectedMints()
{
...
void ZXdhControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

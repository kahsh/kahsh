...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::KSH, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (kahsh.conf).");
            QMessageBox::warning(this, "Kahsh Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#b5b5b5; background-color:#3c4043;}\n a, span { font-family: monospace; }\n span.addr {color:#3c4043; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid white; background-color: #3c4043;}\n td.d0 {font-weight: bold; color:#b5b5b5;}\n h2, h3 { white-space:nowrap; color:#b5b5b5;}\n a { color:#88f6f6; text-decoration:none; }\n a.nav {color:#b5b5b5;}\n";

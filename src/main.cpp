...
#if defined(NDEBUG)
#error "Dilithium cannot be compiled without assertions."
#endif
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zxdh");
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 425 * COIN;
    } else if (nHeight < 1 * 262800 && nHeight >= Params().LAST_POW_BLOCK()) {
        nSubsidy = 1.20 * COIN;
    } else if (nHeight < 2 * 262800 && nHeight >= 1 * 262800) {
        nSubsidy = 0.84 * COIN;
    } else if (nHeight < 3 * 262800 && nHeight >= 2 * 262800) {
        nSubsidy = 0.58 * COIN;
    } else if (nHeight < 4 * 262800 && nHeight >= 3 * 262800) {
        nSubsidy = 0.75 * COIN;
    } else if (nHeight < 5 * 262800 && nHeight >= 4 * 262800) {
        nSubsidy = 0.97 * COIN;
    } else if (nHeight < 6 * 262800 && nHeight >= 5 * 262800) {
        nSubsidy = 1.28 * COIN;
    } else if (nHeight < 7 * 262800 && nHeight >= 6 * 262800) {
        nSubsidy = 1.65 * COIN;
    } else if (nHeight < 8 * 262800 && nHeight >= 7 * 262800) {
        nSubsidy = 2.16 * COIN;
    } else if (nHeight < 9 * 262800 && nHeight >= 8 * 262800) {
        nSubsidy = 1.51 * COIN;
    } else if (nHeight < 10 * 262800 && nHeight >= 9 * 262800) {
        nSubsidy = 1.06 * COIN;
    } else if (nHeight < 11 * 262800 && nHeight >= 10 * 262800) {
        nSubsidy = 0.74 * COIN;
    } else if (nHeight < 12 * 262800 && nHeight >= 11 * 262800) {
        nSubsidy = 0.52 * COIN;
    } else if (nHeight < 13 * 262800 && nHeight >= 12 * 262800) {
        nSubsidy = 0.36 * COIN;
    } else if (nHeight < 105 * 262800 && nHeight >= 13 * 262800) {
        nSubsidy = 0.25 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Dilithium
...
{
    RenameThread("dilithium-scriptch");
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zXdhSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (IsSporkActive(SPORK_18_STAKING_ENFORCEMENT) && block.GetBlockTime() >= GetSporkValue(SPORK_18_STAKING_ENFORCEMENT)) {
            ...
            if (chainActive.Height() - pindex->nHeight + 1 < Params().Stake_MinConfirmations())
            ...
            do {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : xdhInputs) {
                ...
                for (const CTxIn& zXdhInput : zXDHInputs) {
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zxdh");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Dilithium: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zxdh", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zxdh", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);

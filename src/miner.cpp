...
// Note: The cache is initialized in AccumulatorCheckpoints::LoadCheckpoints,
// if no json checkpoints are specified!
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    bool fZerocoinActive = nHeight >= Params().Zerocoin_StartHeight() || GetAdjustedTime() >= Params().Zerocoin_StartTime();
    if (fZerocoinActive)
...

    {
        ...
        for (map<uint256, CTxMemPoolEntry>::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                if (tx.IsZerocoinSpend()) {
                    ...
                    //Priority = (age^6+100000)*amount - gives higher priority to zxdhs that have been in mempool long
                    //and higher priority to zxdhs that are large in value
                    ...
                    // zXDH spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zXDH spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zXDH serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
        ...
        if (fZerocoinActive && pindexPrev->nHeight + 1 >= Params().Zerocoin_StartHeight()) {
            uint256 nCheckpoint;
            ...
            if (nHeight >= pCheckpointCache.first || pCheckpointCache.second.first != hashBlockLastAccumulated) {
                // Note: if Zerocoin_StartHeight() != Zerocoin_Block_V2_Start(), pCheckpointCache
                // was NOT initialized properly!
                assert(Params().Zerocoin_StartHeight() == Params().Zerocoin_Block_V2_Start());
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zXDH with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    return NULL;
                }

                //Sign block with the zXDH key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("BitcoinMiner(): Signing new block with zXDH key failed \n");
                    return NULL;
                }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("DilithiumMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake())
            pwalletMain->zxdhTracker->RemovePending(pblock->vtx[1].GetHash());
        return error("DilithiumMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("DilithiumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dilithium-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running DilithiumMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
